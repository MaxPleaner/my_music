(function() {
  var addButtonToShowAll, buildNavbarTagsMenu, downloadAllButtonOnClick, filterGrid, gridItemOnClick, gridItemOnMouseenter, gridItemOnMouseleave, hideAllContent, isotopeFilterFn, loadInitialState, metadataOnClick, refreshGrid, setupGrid, setupMetadata, showAllButtonOnClick, togglingContentOnMouseenter, togglingContentOnMouseleave;
  ...
  buildNavbarTagsMenu = function($grid, $metadata) {
    ...
    tags.forEach(function(tag) {
      var tagLink;
      tagLink = $("<a></a>").html(tag).addClass("tagLink").attr("href", "#").attr("tag-name", tag);
  ...
  downloadAllButtonOnClick = function($grid, e) {
    var $el;
    e.preventDefault();
    $el = $(e.currentTarget);
    return $.each($el.parents(".content").find(".download"), function(idx, btn) {
      return btn.click();
    });
  };
